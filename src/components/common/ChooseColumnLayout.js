...
const ChooseColumnLayout = () => {
  ...
  const _setCurrentEditable = useCallback(
  ...
  const _setEditable = useCallback(() => {
    if (!_row || _row?.id === _editableRowId || !mounted) return;
    else {
      _setCurrentEditable();
      _handleClose();
    }
  ...
  return (
    <div>
      ...
      <div className={classes.columnContainer}>
        <div onClick={() => _addRow(1)} className={classes.addRow}>
          <Square width='40' height='40' className={classes.icon} />
          <Typography className={classes.text}>Column</Typography>
        </div>
        <div onClick={() => _addRow(2)} className={classes.addRow}>

...
import { setCurrentEditable, removeElement } from 'redux/actions'
...
const EditDevider = ({ id }) => {
    ...
    const deleteElement = () => {
        dispatch(removeElement(_currentElementId, _column.id))
        dispatch(setCurrentEditable(_column))
    }
    return (
        <div>
            <div className={classes.row} onClick={onEdit}>
                <div>
                    ...
                    <Typography variant='h6' className={classes.text}>Devider {elementIndex}</Typography>
                </div>
            </div>
            <div>
                {_currentEditable && (
